 ...
 */
public class DjExpressionParser implements Parser {
    ...
    static final int         states[][] = {
                                        // 0.空格, 1.字母, 2.数字, 3.点号, 4.双引号, 5.单引号, 6.反单引号, 7.反斜线, 8.括号, 9.其它,10|
            /* 0.起始  */{ 0, 1, 2, 5, 7, 9, 11, 4, 6, 4, 14 }, // 初始状态或上一片断刚接收完成状态
            /* 1.变量  */{ B1, 1, 1, B1, E, E, E, B1, B1, B1, B1 }, // 变量名识别
            /* 2.数字  */{ B1, 2, 2, 13, E, E, E, B1, B1, B1, B1 }, // 数字识别
            /* 3.小数  */{ B1, 3, 3, B1, E, E, E, B1, B1, B1, E }, // 小数点号识别
            /* 4.操作*/{ B1, B1, B1, 4, B1, B1, B1, 4, B1, 4, E }, // 操作符识别
            /* 5.点号  */{ B1, 1, 3, B, B1, B1, B1, B1, B1, 4, E }, // 属性点号
            /* 6.括号  */{ B1, B1, B1, B1, B1, B1, B1, B1, B1, B1, B1 }, // 括号
            /* 7.字符*/{ 7, 7, 7, 7, B, 7, 7, 8, 7, 7, 7 }, // 双引号字符串识别
            /* 8.转义  */{ 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7 }, // 双引号字符串转义
            /* 9.字符*/{ 9, 9, 9, 9, 9, B, 9, 10, 9, 9, 9 }, // 单引号字符串识别
            /*10.转义  */{ 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9 }, // 单引号字符串转义
            /*11.字符  */{ 11, 11, 11, 11, 11, 11, B, 12, 11, 11, 11 }, // 反单引号字符串识别
            /*12.转义  */{ 11, 11, 11, 11, 11, 11, 11, 11, 11, 11, 11 }, // 反单引号字符串转义
            /*13.数点  */{ B2, B2, 3, B2, B2, B2, B2, B2, B2, B2, B2 }, // 数字属性点号识别, 区分于小数点(如: 123.toString 或 11..15)
            /* 14.|  */{ B1, 1, 3, 3, B1, B1, B1, B1, B1, 4, B }, // 操作符
                                        };
    ...
    private static final Set<String> BINARY_OPERATORS = new HashSet<String>(Arrays.asList(new String[] { "+", "-", "*",
            "/", "%", "==", "!=", ">", ">=", "<", "<=", "gt", "ge", "lt", "le", "&&", "||", "&", /*"|",*/"^", ">>",
            "<<", ">>>", ",", "?", ":", "instanceof", "is", "[", ".." }));

    private static final Set<String> UNARY_OPERATORS  = new HashSet<String>(Arrays.asList(new String[] { "+", "-", "!",
            "~", "new", "["                          }));
    ...
    private BinaryOperator createBinaryOperator(String name, int priority, int offset) {
        ...
        } else if (StringUtils.isFunction(name) || name.startsWith("|")) {
    ...
    public Expression parse(String source, int offset) throws ParseException {
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            if ("new".equals(msg)) {
                ...
                StringBuilder buf = new StringBuilder();
                buf.append(msg);
                while (i + 2 < tokens.size() && isPackageName(tokens.get(i + 1).getMessage().trim())
                ...
                if (i < tokens.size() - 1) {
                    ...
                    if ("(".equals(next)) {
                        ...
                        if ("(".equals(prev) && ("(".equals(after) || StringUtils.isNamed(after))) {
                            ...
                            UnaryOperator operator = createUnaryOperator(msg, getPriority(msg, true),
                                getTokenOffset(token));
            ...
            // ================
            if (msg.length() >= 2
                && (msg.startsWith("\"") && msg.endsWith("\"") || msg.startsWith("\'") && msg.endsWith("\'") || msg
                    .startsWith("`") && msg.endsWith("`"))) {
                ...
                parameterStack.push(new Constant("true".equals(msg) ? Boolean.TRUE : Boolean.FALSE, false, token
                    .getOffset()));
                beforeOperator = false;
            } else if ("TRUE".equals(msg) || "FALSE".equals(msg)) {
                parameterStack.push(new Constant("TRUE".equals(msg) ? Boolean.TRUE : Boolean.FALSE, true, token
                    .getOffset()));
                beforeOperator = false;
            } else if (msg.length() > 1 && msg.startsWith("@")) {
                parameterStack.push(new Constant(ClassUtils.forName(importPackages, msg.substring(1).trim()), false,
                    token.getOffset()));
                beforeOperator = false;
                    ...
                    if (forbidEqualsMethods != null) {
                        for (String forbid : forbidEqualsMethods) {
                            if (method.equals(forbid)) {
                                throw new ParseException("Forbid call method " + method + " by forbid.method=" + forbid
                                                         + " config.", offset);
                            }
                        }
                    }
                    ...
                    if (forbidEndsMethods != null) {
                        for (String forbid : forbidEndsMethods) {
                            if (method.endsWith(forbid)) {
                                throw new ParseException("Forbid call method " + method + " by forbid.method=*"
                                                         + forbid + " config.", offset);
                            }
                        }
                    }
                }
                ...
                if (beforeOperator) {
                    ...
                    if (!StringUtils.isFunction(msg) && !BINARY_OPERATORS.contains(msg) && !msg.startsWith("|")) {
                        throw new ParseException("Unsupported binary operator " + msg, getTokenOffset(token));
                    }
                ...
                beforeOperator = true;
                // 给无参函数自动补上null参数
                if (msg.startsWith("new ") || StringUtils.isFunction(msg) || (!msg.equals("||") && msg.startsWith("|"))) {
                    boolean miss = i == tokens.size() - 1 || !"(".equals(tokens.get(i + 1).getMessage().trim());
                    boolean empty = i < tokens.size() - 2 && "(".equals(tokens.get(i + 1).getMessage().trim())
                                    && ")".equals(tokens.get(i + 2).getMessage().trim());
                    if (miss || empty) {
    ...
    private Operator popOperator(LinkedStack<Expression> parameterStack, LinkedStack<Operator> operatorStack,
                                 Map<Operator, Token> operatorTokens, int offset) throws ParseException {
        ...
        if (operator instanceof BinaryOperator) {
            ...
            if (parameterStack.isEmpty())
                throw new ParseException("Binary operator " + binaryOperator.getName() + " miss parameter",
                    token == null ? offset : getTokenOffset(token));
            ...
            if (parameterStack.isEmpty())
                throw new ParseException("Unary operator " + unaryOperator.getName() + "miss parameter",
                    token == null ? offset : getTokenOffset(token));
