...
public class DefaultEngine extends Engine {
    ...
    public Map<String, Object> createContext(final Map<String, Object> parent, Map<String, Object> current) {
        return new DelegateMap<String, Object>(parent, current) {
            ...
            public Object get(Object key) {
                ...
                if (value == null && parent == null && resolver != null) {
     ...
     */
    @SuppressWarnings("unchecked")
    public Template getTemplate(String name, Locale locale, String encoding, Object args) throws IOException,
                                                                                         ParseException {
        ...
        if (reference == null) {
            if (cache instanceof ConcurrentMap) {
                ...
                VolatileReference<Template> old = (VolatileReference<Template>) ((ConcurrentMap<Object, Object>) cache)
                    .putIfAbsent(key, reference);
    ...
    private Template parseTemplate(Resource resource, String name, Locale locale, String encoding, Object args)
                                                                                                               throws IOException,
                                                                                                               ParseException {
        if (resource == null) {
            resource = loadResource(name, locale, encoding);
        ...
        try {
            ...
            Map<String, Class<?>> parameterTypes = useRenderVariableType && args != null ? new DelegateMap<String, Class<?>>(
                new TypeMap(convertMap(args))) : null;
            Template template = translator.translate(resource, root, parameterTypes);
            if (template instanceof AbstractTemplate
                && Context.getContext().containsKey(DirectiveEnum.EXTENDS.getCode())) {
                String extendsName = (String) Context.getContext().remove(DirectiveEnum.EXTENDS.getCode());
                ((AbstractTemplate) template).setParent(getTemplate(extendsName, locale, encoding,
                    args));
            }
            if (logger != null && logger.isDebugEnabled()) {
                logger.debug("Parsed the template " + name + ", eslapsed: " + (System.currentTimeMillis() - start)
                             + "ms.");
            }
            return template;
        ...
        if (parameters == null || parameters instanceof Map) {
            ...
            throw new IllegalArgumentException(
                "No such "
                        ...
                        + " to Map. Please check engine.getTemplate() args or implement a converter and add config in httl.properties: map.converter+=com.your."
                        + parameters.getClass().getSimpleName() + Converter.class.getName() + ".");
        }
    }
        ...
        if (logger != null && StringUtils.isNotEmpty(name)) {
            if (logger.isWarnEnabled() && !ConfigUtils.isFilePath(name)) {
                try {
                    ...
                    if (realPaths.size() > 1) {
                        logger.warn("Multi httl config in classpath, conflict configs: " + realPaths
                                    + ". Please keep only one config.");
                    }
                } catch (IOException e) {
            ...
            if (logger.isInfoEnabled()) {
                ...
                if (StringUtils.isNotEmpty(realPath)) {
                    logger.info("Load httl config from " + realPath + " in "
                                + (name.startsWith("/") ? "filesystem" : "classpath") + ".");
                }
            }
        }
    }
        ...
        if (preload) {
            try {
                ...
                if (logger != null && logger.isInfoEnabled()) {
                    logger.info("Preload " + count + " templates from directory "
                                + (templateDirectory == null ? "/" : templateDirectory) + " with suffix "
                                + Arrays.toString(templateSuffix));
                }
            } catch (Exception e) {
                if (logger != null && logger.isErrorEnabled()) {
                    logger.error(e.getMessage(), e);
